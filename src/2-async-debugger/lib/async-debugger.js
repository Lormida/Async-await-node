...
/**
 * Invoke a callback function when
...
export class AsyncDebugger {
  ...
  traverse(indentLevel, asyncOp) {
    const lines = [`${asyncOp.asyncId} - ${asyncOp.type} - ${asyncOp.status}`].concat(
  ...
  addAsyncOperation(asyncId, type, triggerAsyncId, resource) {
    ...
    fs.writeSync(
      1,
      `%% add ${asyncId}, type = ${type}, parent = ${triggerAsyncId}, context = ${executionContextId}, ${this.labelMap.get(
        executionContextId
      )} #ops = ${this.numAsyncOps.get(executionContextId)}, total #ops = ${
        this.curAsyncOps.size
      }\n`
    )
  ...
  removeAsyncOperation(asyncId, reason) {
    ...
    if (executionContextId !== undefined) {
      ...
      if (numAsyncOps !== undefined) {
        ...
        fs.writeSync(
          1,
          `%% remove ${asyncId}, reason = ${reason}, context = ${executionContextId}, ${this.labelMap.get(
            executionContextId
          )} #ops = ${this.numAsyncOps.get(executionContextId)}, total #ops = ${
            this.curAsyncOps.size
          }\n`
